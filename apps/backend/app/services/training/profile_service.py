...
def _get_user_records(payload: dict[str, Any], user_id: str) -> list[dict[str, Any]]:
    user_bucket = payload.setdefault("users", {}).get(user_id, [])
    if isinstance(user_bucket, list):
        return user_bucket
    if isinstance(user_bucket, dict):
        records = user_bucket.get("records", [])
...
def append_user_record(user_id: str, record: dict[str, Any]) -> dict[str, Any]:
    payload = _load_payload()
    records = _get_user_records(payload, user_id)
...
def get_user_records(user_id: str) -> list[dict[str, Any]]:
    payload = _load_payload()
    return list(_get_user_records(payload, user_id))
...
def get_progress_snapshot(user_id: str, total_cases: int) -> dict[str, Any]:
    ...
    correct_count = sum(1 for score in scores if score >= 100)
    mistake_count = sum(
        1
...
def get_user_profile(user_id: str) -> dict[str, Any]:
    ...
    for ability in ABILITY_DIMENSIONS:
        if answered_count == 0:
            ...
            mastery = max(0.0, 1 - (error_counts[ability] / answered_count))
        abilities.append(
            {
                "ability": ability,
    ...
    weakest = [
        item["ability"]
        for item in sorted(abilities, key=lambda item: (item["error_count"] * -1, item["mastery"]))
